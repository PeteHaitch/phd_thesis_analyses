...
betaBoxPlot <- function(beta_reduced, dataset, min_cov, ncol, nrow) {
  ...
  g <- ggplot(beta_reduced, aes(x = type, ymin = stats1, lower = stats2,
    ...
    ggtitle(paste0(dataset, ": min_cov = ", min_cov))
  g
}
...
patternFreqsPlot <- function(pattern_freqs_reduced, dataset, min_cov, ncol, 
                             nrow) {
  h <- grep("^h[0-9]", colnames(pattern_freqs_reduced), value = TRUE)
  setkey(pattern_freqs_reduced, sample, type)
  h <- parse(text = paste0(h, "\n"))
  x <- bplapply(h, function(h_i, pattern_freqs_reduced) {
    pattern_freqs_reduced[, as.list(bps(eval(h_i), N)), 
                          by = key(pattern_freqs_reduced)][, 
                                                           "h" := deparse(h_i)]
    }, pattern_freqs_reduced = pattern_freqs_reduced)
  x <- rbindlist(x)
  
  g <- ggplot(x, aes(x = h, ymin = stats1, lower = stats2, middle = stats3,
                     upper = stats4, ymax = stats5, 
                     group = interaction(h, type), colour = type)) +
    geom_boxplot(stat = "identity") + 
    facet_wrap(~ sample, ncol = ncol, nrow = nrow) +
    thesis_theme + 
    scale_colour_brewer(palette = "Dark2") +
...
lor_quantiles_by_pm_boxplots_dir <- paste0("pdf/", dataset, 
                                 "/lor_quantiles_by_pm_boxplots/")
dir.create(lor_quantiles_by_pm_boxplots_dir, recursive = TRUE)
ggsave(paste0(lor_quantiles_by_pm_boxplots_dir, dataset,
              "_lor_quantiles_by_pm_boxplots.pdf"), g, height = 9, width = 16)
...
g <- patternFreqsPlot(pattern_freqs_reduced = pattern_freqs_reduced, 
                      ...
                      nrow = nrow)
pattern_freqs_boxplot_by_pm_boxplots_dir <- paste0(
  "pdf/", dataset, "/pattern_freqs_boxplot_by_pm_boxplots/")
dir.create(pattern_freqs_boxplot_by_pm_boxplots_dir, recursive = TRUE)
ggsave(paste0(pattern_freqs_boxplot_by_pm_boxplots_dir, dataset,
              "_pattern_freqs_boxplot_by_pm_boxplots.pdf"), g, height = 9, 
       width = 16)
