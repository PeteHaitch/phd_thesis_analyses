...
op <- par()
par(mfrow = c(3, 3), cex.main = 1.3, cex.lab = 1.2, cex.lab = 1.2)
```
...
__TODO: Re-write using ggplot and `thesis_theme`__
...
plotMH <- function(mh_by_ipd_seqnames, mh_by_ipd, dataset, nrow, ncol, xmax, 
  ...
  seqtype <- vector("character", length(uniq_seqnames))
  autosomes <- grep("[0-9]", uniq_seqnames)
  seqtype[autosomes] <- "autosome"
  ...
  seqtype[MT] <- "MT"
  names(seqtype) <- uniq_seqnames
  mh_by_ipd_seqnames[, seqtype := seqtype[seqnames]]
  ...
  if (annotate) {
    g <- g + geom_point(aes(col = seqtype), alpha = 0.25)
  } else {
...
plotMH(mh_by_ipd_seqnames, mh_by_ipd, dataset, nrow, ncol, 350, "adjacent") +
  ylim(-2.5, 7.5) + 
  ylab(bquote(atop(psi, "Truncated [-2.5, 7.5]")))
```

__TODO: There's no data from chrM for E18BUF; why?__
...
__TODO: Something seems to have gone a little haywire on chr21 of a few of these samples__.
